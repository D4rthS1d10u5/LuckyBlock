...
import com.LuckyBlock.Resources.IDebug;
...
public class ColorsClass {
    ...
    protected static final void sendMessage(CommandSender sender, String loc, TextAction[] actions, ColorsClass.ObjectType... objectTypes) {
        String s = getMessage(loc, objectTypes);
        if (s != null && !s.equalsIgnoreCase("")) {
            if (sender instanceof Player) {
                RawText _a = new RawText("");
                RawText pl = new RawText(green + "[" + yellow + bold + "LB" + green + "]");
                pl.addAction(new TextAction(EnumTextEvent.HOVER_EVENT, EnumTextAction.SHOW_TEXT, yellow + "Lucky Block"));
                RawText text1 = new RawText(": ");
                RawText text2 = new RawText(s);
                TextAction[] var12 = actions;
                int var11 = actions.length;

                for(int var10 = 0; var10 < var11; ++var10) {
                    TextAction a = var12[var10];
                    if (a != null) {
                        text2.addAction(a);
                    }
                }

                TellRawSender.sendTo((Player)sender, new RawText[]{_a, pl, text1, text2});
            } else {
        ...
        if (string != null) {
            ...
            if (!string.equalsIgnoreCase("")) {
                if (sender instanceof Player) {
                    ...
                    pl.addAction(new TextAction(EnumTextEvent.HOVER_EVENT, EnumTextAction.SHOW_TEXT, yellow + "Lucky Block"));
                    RawText text1 = new RawText(": ");
                    RawText text2 = new RawText(string);
                    TellRawSender.sendTo((Player)sender, new RawText[]{_a, pl, text1, text2});
                } else {
